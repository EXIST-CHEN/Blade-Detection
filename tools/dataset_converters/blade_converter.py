...
from .blade_data_utils import WaymoInfoGatherer, get_blade_image_info
from .nuscenes_converter import post_process_coords

blade_categories = ('Blade', )
...
def create_blade_info_file(data_path,
                           pkl_prefix='blade',
                           with_plane=False,
                           save_path=None,
                           relative_path=True):
    """Create info file of Blade dataset.
    ...
    blade_infos_train = get_blade_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    _calculate_num_points_in_gt(data_path, blade_infos_train, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'Blade info train file is saved to {filename}')
    mmengine.dump(blade_infos_train, filename)
    blade_infos_val = get_blade_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
        ...
        relative_path=relative_path)
    _calculate_num_points_in_gt(data_path, blade_infos_val, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_val.pkl'
    print(f'Blade info val file is saved to {filename}')
    mmengine.dump(blade_infos_val, filename)
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'Blade info trainval file is saved to {filename}')
    mmengine.dump(blade_infos_train + blade_infos_val, filename)

    blade_infos_test = get_blade_image_info(
        data_path,
        training=False,
        label_info=False,
        velodyne=True,
    ...
    print(f'Blade info test file is saved to {filename}')
    mmengine.dump(blade_infos_test, filename)
...
def _create_reduced_point_cloud(data_path,
    ...
    Args:
        ...
        front_camera_id (int, optional): The referenced/front camera ID.
            Default: 2.
    """
    blade_infos = mmengine.load(info_path)

    for info in mmengine.track_iter_progress(blade_infos):
        pc_info = info['point_cloud']
        image_info = info['image']
        calib = info['calib']
...
def export_2d_annotation(root_path, info_path, mono3d=True):
    ...
    blade_infos = mmengine.load(info_path)
    cat2Ids = [
        dict(id=blade_categories.index(cat_name), name=cat_name)
        for cat_name in blade_categories
    ]
    coco_ann_id = 0
...
def get_2d_boxes(info, occluded, mono3d=True):
    ...
    for ann_idx, ann_rec in enumerate(ann_recs):
        ...
        if mono3d and (repro_rec is not None):
            ...
            repro_rec['velo_cam3d'] = -1  # no velocity in Blade
            ...
            repro_rec['attribute_name'] = -1  # no attribute in Blade
...
def generate_record(ann_rec, x1, y1, x2, y2, sample_data_token, filename):
    ...
    if repro_rec['category_name'] not in blade_categories:
        return None
    cat_name = repro_rec['category_name']
    coco_rec['category_name'] = cat_name
    coco_rec['category_id'] = blade_categories.index(cat_name)
