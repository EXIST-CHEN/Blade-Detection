...
from mmdet3d.datasets import BladeDataset
from mmdet3d.structures import Det3DDataSample, LiDARInstance3DBoxes


def _generate_blade_dataset_config():
    data_root = 'tests/data/blade'
    ann_file = 'blade_infos_train.pkl'
    classes = ['Blade']
...
def test_getitem():
    ...
    data_root, ann_file, classes, data_prefix, \
        pipeline, modality, = _generate_blade_dataset_config()
    modality['use_camera'] = True

    blade_dataset = BladeDataset(
        data_root,
        ann_file,
        data_prefix=dict(
    ...
    blade_dataset.prepare_data(0)
    input_dict = blade_dataset.get_data_info(0)
    blade_dataset[0]
    ...
    ann_info = blade_dataset.parse_ann_info(input_dict)
    ...
    blade_blade_dataset = BladeDataset(
        data_root,
        ann_file,
        data_prefix=dict(
            pts='training/velodyne_reduced',
        ...
        pipeline=pipeline,
        metainfo=dict(classes=['Blade']),
        modality=modality)

    input_dict = blade_blade_dataset.get_data_info(0)
    ann_info = blade_blade_dataset.parse_ann_info(input_dict)
    ...
    assert len(blade_blade_dataset.metainfo['classes']) == 1
