...
from mmdet3d.evaluation import blade_eval
...
class BladeMetric(BaseMetric):
    """Blade evaluation metric.
    ...
    def __init__(self,
        ...
        self.default_prefix = 'Blade metric'
        super(BladeMetric, self).__init__(
    ...
    def convert_annos_to_blade_annos(self, data_infos: dict) -> list:
        """Convert loading annotations to Blade annotations.
        ...
        Returns:
            List[dict]: List of Blade annotations.
        """
            ...
            for i, annos in enumerate(data_annos):
                if len(annos['instances']) == 0:
                    ...
                    for instance in annos['instances']:
                        ...
                        blade_annos['name'].append(label2cat[label])
                        blade_annos['truncated'].append(instance['truncated'])
                        blade_annos['occluded'].append(instance['occluded'])
                        blade_annos['alpha'].append(instance['alpha'])
                        blade_annos['bbox'].append(instance['bbox'])
                        blade_annos['location'].append(instance['bbox_3d'][:3])
                        blade_annos['dimensions'].append(
                            instance['bbox_3d'][3:6])
                        blade_annos['rotation_y'].append(
                            instance['bbox_3d'][6])
                        blade_annos['score'].append(instance['score'])
                    for name in blade_annos:
                        blade_annos[name] = np.array(blade_annos[name])
                data_annos[i]['blade_annos'] = blade_annos
        return data_annos
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        self.data_infos = self.convert_annos_to_blade_annos(pkl_infos)
        ...
        gt_annos = [
            self.data_infos[result['sample_idx']]['blade_annos']
            for result in results
        ]

        for metric in self.metrics:
            ap_dict = self.blade_evaluate(
                result_dict,
                gt_annos,
                metric=metric,
                logger=logger,
    ...
    def blade_evaluate(self,
        ...
        for name in results_dict:
            ...
            ap_result_str, ap_dict_ = blade_eval(
    ...
    def format_results(self,
        ...
        for name in results[0]:
            ...
            if 'pred_instances' in name and '3d' in name and name[
                ...
                result_list_ = self.bbox2result_blade(net_outputs,
                ...
                result_dict[name] = result_list_
                    ...
                    name]:
                net_outputs = [result[name] for result in results]
                result_list_ = self.bbox2result_blade2d(
                    net_outputs, sample_id_list, classes, pklfile_prefix_,
                    submission_prefix_)
                result_dict[name] = result_list_
        return result_dict, tmp_dir

    def bbox2result_blade(self,
                          net_outputs: list,
                          sample_id_list: list,
                          class_names: list,
                          pklfile_prefix: str = None,
                          submission_prefix: str = None):
        """Convert 3D detection results to blade format for evaluation and test
        submission.
    ...
    def bbox2result_blade2d(self,
                            net_outputs: list,
                            sample_id_list,
                            class_names: list,
                            pklfile_prefix: str = None,
                            submission_prefix: str = None):
        """Convert 2D detection results to blade format for evaluation and test
        submission.
        ...
        Returns:
            list[dict]: A list of dictionaries have the blade format
        """
        assert len(net_outputs) == len(self.data_infos), \
            'invalid list length of network outputs'
        det_annos = []
