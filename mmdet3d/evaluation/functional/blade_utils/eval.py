...
def calculate_iou_partly(dt_annos, gt_annos, metric, num_parts=50):
    ...
    Args:
        dt_annos (dict): Must from get_label_annos() in blade_metric.py.
        gt_annos (dict): Must from get_label_annos() in blade_metric.py.
...
def eval_class(gt_annos,
               ...
               compute_aos=False,
               num_parts=200):
    """Blade eval. support 2d/bev/3d/aos eval. support 0.5:0.05:0.95 coco AP.

    Args:
        gt_annos (dict): Must from get_label_annos() in blade_metric.py.
        dt_annos (dict): Must from get_label_annos() in blade_metric.py.
        current_classes (list[int]): 0: blade.
...
def blade_eval(gt_annos,
    ...
    if len(current_classes) > 1:
        ...
        for idx in range(3):
            ...
            if mAP11_3d is not None:
                ret_dict[f'BLADE/Overall_3D_AP11_{postfix}'] = mAP11_3d[idx, 0]
            if mAP11_bev is not None:
                ret_dict[f'BLADE/Overall_BEV_AP11_{postfix}'] =\
                    mAP11_bev[idx, 0]
            if mAP11_bbox is not None:
                ret_dict[f'BLADE/Overall_2D_AP11_{postfix}'] =\
                    mAP11_bbox[idx, 0]
    ...
    if len(current_classes) > 1:
        ...
        for idx in range(3):
            postfix = f'{difficulty[idx]}'
            if mAP40_3d is not None:
                ret_dict[f'BLADE/Overall_3D_AP40_{postfix}'] = mAP40_3d[idx, 0]
            if mAP40_bev is not None:
                ret_dict[f'BLADE/Overall_BEV_AP40_{postfix}'] =\
                    mAP40_bev[idx, 0]
            if mAP40_bbox is not None:
                ret_dict[f'BLADE/Overall_2D_AP40_{postfix}'] =\
                    mAP40_bbox[idx, 0]

    return result, ret_dict


def blade_eval_coco_style(gt_annos, dt_annos, current_classes):
    """coco style evaluation of blade.
